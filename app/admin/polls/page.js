...
export default function AdminPollsPage() {
  ...
  const fetchPolls = async () => {
    ...
    if (res.ok) {
      ...
      setPollsEnabled(data.polls_enabled !== false);
      // For admin list, fetch all polls
    }
    // Load all polls (admin)
    const allRes = await fetch('/api/admin/polls');
    if (allRes.ok) {
      const allData = await allRes.json();
      setPolls(allData.polls ?? []);
    }
    setLoading(false);
  };
  ...
  const handleToggleEnabled = async () => {
    ...
    const res = await fetch('/api/polls', {
      ...
      body:    JSON.stringify({ polls_enabled: next }),
    });
    if (res.ok) {
  ...
  const handleCreatePoll = async (e) => {
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      return setMsg({ type: 'error', text: data.error ?? 'Failed to create.' });
    }
  ...
  const handleSetActive = async (id) => {
    const res = await fetch('/api/polls', {
      method:  'PUT',
      headers: { 'Content-Type': 'application/json' },
      body:    JSON.stringify({ activePollId: id }),
    });
    if (res.ok) {
      ...
      <div className="flex flex-wrap justify-between items-center gap-4">
        ...
        >
          ...
          <span className="flex items-center gap-2">
            <span className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${pollsEnabled ? 'bg-green-500' : 'bg-red-400'}`}>
              <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${pollsEnabled ? 'translate-x-4' : 'translate-x-1'}`} />
            </span>
        ...
        <div className="card p-6 md:col-span-1 border-brand-100 bg-brand-50/20">
          ...
          <form onSubmit={handleCreatePoll} className="space-y-4">
            <div>
              ...
              <input type="text" value={newQuestion} onChange={e => setNewQuestion(e.target.value)} placeholder="Ex: What percentage of our body is water?" className="input" required />
            </div>
            <div>
              <label className="label">Options *</label>
              {options.map((opt, i) => (
                <div key={i} className="flex gap-2 mb-2">
                  <input type="text" value={opt} onChange={(e) => { const n = [...options]; n[i] = e.target.value; setOptions(n); }} placeholder={`Option ${i + 1}`} className="input py-2" required={i < 2} />
                  {options.length > 2 && (
              ...
              <button type="button" onClick={() => setOptions([...options, ''])} className="btn-secondary w-full text-xs py-2 mt-1 border-dashed">
                + Add Another Option
              </button>
            </div>
            <button type="submit" className="btn-primary w-full mt-4">Save Poll to Pool</button>
          </form>
        </div>
                  ...
                  return (
                    <div key={opt.id} className="relative">
                      ...
                      <div className="absolute inset-0 bg-surface-secondary rounded-lg overflow-hidden">
                        <div className="h-full bg-brand-100/50" style={{ width: `${percent}%` }} />
                      </div>
                    </div>
                  );
                })}
